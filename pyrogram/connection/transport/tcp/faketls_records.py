...
"""The TLS record layer an ``ee``-prefixed MTProxy speaks once it has answered.
...
outside. Mirrors TDLib's ``ObfuscatedTransport``, whose reader rejects any
record that is not application data.

https://github.com/tdlib/td/blob/d1085f9/td/mtproto/TcpTransport.cpp#L173-L214
"""
...
# The five bytes every record opens with: content type 0x17 for application
# data, the legacy version 0x0303, then a 2-byte big-endian length.
...
# application record. Both go into the digest, so both are read rather than
# skipped.
...
class FakeTlsRecords:
    """Cuts the obfuscated2 stream into TLS application-data records, and back."""

    def __init__(self, read_exactly: ReadExactly, *, prologue: bytes) -> None:
        self._read_exactly = read_exactly

        # The obfuscated2 header rides inside the first record rather than in
        # one of its own: a lone 64-byte record right after a TLS handshake is a
    ...
    async def recv(self, length: int) -> bytes | None:
        # A record boundary has nothing to do with a read boundary, so reads are
    ...
    async def _read_record(self) -> bytes | None:
        ...
        if header[: len(APPLICATION_DATA_PREFIX)] != APPLICATION_DATA_PREFIX:
            raise OSError(f"fake-TLS: expected an application-data record, got {header.hex()}")
