...
- `f81a62a4` — `__version__` was raised past the `py-tgcalls` floor. That package declares
  ...
  not a ceiling**, so the version is now `3.3.2` and no longer pinned. Keep it `>=1.2.20`.
...
- Branch `dev`, package `pyrogram`, `__version__` `3.3.2`.
...
- Test suite: **5886 tests** across `tests/{unit,contract,integration}/`; coverage of the
...
- Session storage: WAL, busy timeout 15s (`SQLiteStorage.BUSY_TIMEOUT`, was sqlite3's 5s
  default). `open()` no longer `VACUUM`s every existing session file — only after a migration
  actually runs. `sqlite3.OperationalError: database is locked` means the busy handler waited its
  full timeout, **not** that there was no timeout; it needs two connections on one session file,
  so rule out a second `Client` on the same file before touching the library. Every write method
  commits: sqlite holds the WAL write lock from a transaction's first statement until commit, and
  `set_update_state` used to leave it open until the watchdog's `save()` fifteen minutes later.
